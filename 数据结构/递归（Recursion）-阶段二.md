...
**尾递归可以轻松转换为循环（迭代）**，而且很多编译器会自动进行 **尾递归优化（Tail Call Optimization, TCO）**——把递归转成循环，从而：

- ✅ 不需要额外的栈空间（空间从 $O(n)$ 降为 $O(1)$）
- ✅ 不会栈溢出
- ✅ 通常更快
...
<details>
<summary><strong>点击展开完整练习代码</strong></summary>
