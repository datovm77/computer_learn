...
// ptr[0] = 'h';  // ❌ 编译错误：ptr 指向 const char
// 若强制去掉 const 后修改字符串字面量，行为才是未定义的。
```
...
### 9.2 核心思想：用一个无符号整数的每一位代表一个字母

小写字母一共 26 个（`a~z`）。常见平台的 `unsigned int` 有 32 个值位，但 C++ 标准并不保证它一定是 32 位；本例要求它至少有 26 个值位。我们可以让每一位代表一个字母是否已经出现过：
...
**这样，一个具有至少 26 个值位的无符号整数就相当于一个能追踪 26 个字母的"存在表"！**
...
static_assert(std::numeric_limits<unsigned int>::digits >= 26,
              "该位图实现需要 unsigned int 至少有 26 个值位");
...
| 比较维度   | 计数数组                 | 位运算                         |
| ---------- | ------------------------ | ------------------------------ |
| 空间       | 26 个 `int`（大小依平台而定） | 1 个 `unsigned int`（常见平台为 4 字节） |
| 能统计次数 | ✅ 是                     | ❌ 否（只能判断是否重复）       |
| 适用字符集 | 任意大小                 | 至多为无符号整数的值位数；本例需至少 26 位 |
| 代码复杂度 | 简单直观                 | 需要理解位运算                 |
...
当 n 个字符彼此不同，排列总数为 **n!（n 的阶乘）**：
- 3 个字符 → 3! = 6 种排列
- 4 个字符 → 4! = 24 种排列
- 5 个字符 → 5! = 120 种排列

若存在重复字符，不同排列的数量会更少；例如 `"AAB"` 只有 `3! / 2! = 3` 种不同排列。前面的基础递归会仍然输出 `n!` 次，其中包含重复结果；11.7 的版本会跳过这些重复输出。
...
| 指标       | 值            | 说明                         |
| ---------- | ------------- | ---------------------------- |
| 时间复杂度 | **O(n × n!)** | 基础递归枚举 n! 条路径，每次输出需 O(n)；有重复字符时其中部分输出相同 |
| 空间复杂度 | **O(n)**      | 递归调用栈深度为 n           |
| 不同排列数 | **n!**        | 仅在字符互异时；有重复字符时为 `n! / (c₁!c₂!...)` |
