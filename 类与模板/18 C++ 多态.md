...
> 说明：示例默认已包含必要头文件，例如 `<iostream>`、`<string>`、`<memory>`，并默认使用 `std::cout`、`std::string` 等标准库名称。
...
- **静态多态**（编译时多态）：编译期决定调用目标，例如函数重载、运算符重载、模板、CRTP

- **动态多态**（运行时多态）：通过基类指针或引用调用虚函数，根据对象的动态类型决定最终函数 ← **本教程重点**
...
2. **子类重写父类的虚函数**（函数名、参数列表以及 cv/ref 限定要匹配；返回类型通常相同，允许协变返回）

3. **通过基类指针或基类引用调用虚函数**，而不是按值拷贝对象
...
```cpp
void byValue(Animal animal) {
    animal.speak();  // 对象切片：只剩 Animal 子对象
}

void byRef(Animal& animal) {
    animal.speak();  // 动态绑定
}
...
把派生类对象按值传给基类参数时，会发生**对象切片**：派生类特有部分被切掉，只拷贝基类子对象。动态多态应使用基类引用或指针。
...
C++ 标准只规定虚函数的语义，不规定必须使用虚函数表或固定对象布局。主流编译器通常用 `vptr + vtable` 实现动态绑定，下面用这种常见模型帮助理解：
...
2. 派生类如果要实例化，就必须实现继承来的纯虚函数；否则派生类仍然是抽象类
...
public:
    int calculate() override {  // 具体类需要实现纯虚函数
        return m_A + m_B;
    }
};
...
int main() {
    ...
    delete animal;  // 未定义行为；常见表现是只调用 Animal 析构
...
- 只要可能通过父类指针 `delete` 子类对象，父类析构函数就必须是虚析构或纯虚析构

- 纯虚析构函数必须提供定义；普通纯虚函数也可以提供定义，但通常不要求。原因是销毁派生对象时，基类析构函数一定会被调用
...
| 纯虚函数 | 派生类要实例化时必须实现 | virtual void func() = 0  |
| 抽象类   | 包含纯虚函数的类     | 不能实例化               |
...
- 只要可能通过基类指针 `delete` 派生对象，就使用虚析构

❌ **DON'T（不应该做）**：

- 不要将构造函数声明为虚函数

- 不要依赖构造/析构函数中的虚调用实现动态多态

- 普通成员函数不需要都声明为虚函数（有性能开销）

构造函数和析构函数中调用虚函数时，不会表现出“完整对象”的动态多态。构造基类时派生类部分尚未构造；析构基类时派生类部分已经析构，因此虚调用会绑定到当前构造/析构阶段对应的类。

### 8.3 虚函数性能考虑

- 虚函数调用比普通函数慢一点（需要查表）

- 在常见单继承实现中，含虚函数的对象通常需要保存一个 `vptr`；多重继承、虚继承等场景布局可能更复杂
...
class Cat : public Animal {
public:
    void speak() { }  // ❌ 不是 override；这是新函数，并隐藏 Animal::speak(int)
};
```

推荐从第一次重写开始就写 `override`：
...
Animal *a = new Cat();
delete a;  // 未定义行为；常见表现是只调用 Animal::~Animal()，导致 Cat 资源未释放
```
...
3. 多态基类通常应有虚析构；尤其可能通过基类指针删除派生对象时必须如此

多态的本质是**接口复用**和**实现可替换**：调用方依赖稳定的基类接口，具体行为由运行期对象类型决定。
