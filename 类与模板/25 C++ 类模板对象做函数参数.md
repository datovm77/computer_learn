...
将**函数也设计成模板函数**，把类模板中的**一个或多个参数**变为函数模板参数，保持类模板的结构。
...
void test04() {
    ...
    Person<const char*, double> p2("吴九", 28.5);
    printPerson3(p2);  // T 被推导为 Person<const char*, double>
}
```
...
int main() {
    ...
    Person<const char*, double> p3("王五", 25.5);
    printPerson2(p2);
    printPerson2(p3);
