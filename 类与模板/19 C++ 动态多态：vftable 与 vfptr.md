...
> 
>
> - 主流编译器通常会为**含有虚函数的类**生成虚函数表；具体布局和存储位置由实现决定
>
> - 主流实现中，对象通常通过 vfptr 指向其所属类的虚函数表
>
...
下面的代码只能用于观察特定编译器/ABI 下的常见实现，直接解释对象内存和函数指针不是可移植 C++：
...
**原因：** 通过基类指针 `delete` 派生类对象但基类析构函数不是虚函数，标准层面是**未定义行为**；常见表现是只执行 `Animal::~Animal()`。
...
纯虚函数用 `= 0` 声明，使类成为抽象类；它也可以在类外提供定义。

在虚函数表层面，编译器可能用空指针、运行库处理函数或其他方式表示纯虚函数，具体实现不由 C++ 标准规定：

```
Shape::vftable:
┌──────────────────┐
│[0] → 0 / 错误处理函数 │  ← 纯虚函数的表项表示方式由实现决定
│[1] → 0 / 错误处理函数 │  ← 纯虚函数的表项表示方式由实现决定
└──────────────────┘
```

- `Shape` 无法实例化，这是 C++ 语言规则，不依赖某种固定的虚函数表布局

- 子类 **必须** 重写所有纯虚函数，才不是抽象类，才能实例化
...
```
【编译期】主流编译器通常为含虚函数的类生成虚函数表（vftable），常见存放在只读数据段（如 .rdata）
...
> 主流编译器通常会为有 `virtual` 函数的类生成一张 **虚函数表**，对象内部通常有一个 **虚函数表指针**，在构造时指向它所属类的虚函数表。当子类 **重写** 虚函数时，子类的虚函数表中对应位置的地址会被替换成子类的实现。通过父类指针/引用调用虚函数时，程序在运行时通过 vfptr → vftable → 函数地址 这条链路找到正确的函数执行，这就是 **动态绑定**。
...
│  因此：构造函数中调用虚函数 ≠ 多态                │
│                                                 │
│  通过基类指针 delete 派生对象时，基类析构应 virtual │
│  否则是未定义行为，常见表现是子类析构不被调用       │
└─────────────────────────────────────────────────┘
```
