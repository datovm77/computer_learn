...
> 说明：示例默认已包含必要头文件，例如 `<iostream>`、`<string>`、`<stdexcept>`，并默认使用 `std::cout`、`std::ostream` 等标准库名称。
...
**运算符重载本质上是函数调用的语法糖**。当你写 `a + b` 时，可以把它理解为下面两种候选形式之一：
...
实际重载决议会综合成员函数、非成员函数、参数转换和可见性，不是简单机械替换。
...
| 访问私有成员 | 可以直接访问 | 只能通过公开接口访问；若要直接访问私有成员才需要友元 |
| 左操作数类型 | 必须是当前类对象 | 可以是任意类型，但至少一个操作数要是类或枚举类型 |
...
**当左操作数不是当前类对象，且希望它参与重载时，通常使用非成员函数：**
...
**这是非常重要的运算符！** 默认的赋值运算符会逐成员赋值；当类直接拥有裸指针资源时，可能导致多个对象指向同一资源，进而 double delete 或悬空指针。
...
public:
    ...
    // 赋值运算符重载：copy-and-swap，兼顾自赋值与异常安全
...
3. ✅ 深拷贝新数据，避免多个对象共同拥有同一裸指针

4. ✅ 返回 `*this` 的引用（支持链式赋值）

实际项目中更推荐使用 `std::string`、`std::vector`、`std::unique_ptr` 等 RAII 类型，让编译器生成正确的拷贝/移动/析构逻辑。
...
通常用非成员函数重载，因为左操作数是流对象，无法把 `operator<<` 写成 `Point` 的成员函数来支持 `cout << p`。如果只通过公开接口输出，就不需要友元；只有要直接访问私有成员时才声明为友元。
...
`operator->` 的返回值必须是原始指针，或返回另一个也支持 `operator->` 的对象；编译器会继续应用 `->`，直到得到指针。
...
5. **不要改变内置语义预期**：`+` 通常不修改左右操作数，`+=` 通常修改左操作数并返回左操作数引用

6. **C++20 可用 `<=>`**：如果类型存在自然全序或偏序，可以考虑默认或自定义三路比较，让编译器生成部分关系运算符
...
运算符重载应服务于类型的自然语义：让自定义类型像内置类型一样易读、可组合，同时保持返回值、`const` 和资源管理规则一致。
