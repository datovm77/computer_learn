...
> 
> 这里的“不可访问”不是“不被继承”。基类的 `private` 成员仍然存在于派生类对象的基类子对象中，只是派生类代码不能直接读写它，通常要通过基类提供的 `public` / `protected` 成员函数间接访问。

> 默认继承方式：`class Son : Base` 等价于 `class Son : private Base`；`struct Son : Base` 等价于 `struct Son : public Base`。实际写代码时建议显式写出继承方式，避免误解。
...
> `public` 继承表达稳定的 **is-a** 关系，类外可以把派生类对象隐式转换为基类引用/指针；`private` / `protected` 继承会限制这种类外转换，更像实现细节。若只是“拥有一个对象”，通常优先考虑组合而不是继承。
...
> 💡 记忆口诀：**构造从父到子，析构从子到父**。注意：多个基类的构造顺序由继承列表中的声明顺序决定，成员对象的构造顺序由成员变量声明顺序决定，不由初始化列表的书写顺序决定。
...
// ❌ 错误写法
Student(string name, int age, int score) {
    Person(name, age);  // 这只是创建临时对象，不是调用基类构造；本例还会因 Person 无默认构造而编译失败
    m_Score = score;
}
...
如果希望在派生类中保留基类的重载版本，可以使用 `using` 声明：

```cpp
class Son : public Base {
public:
    using Base::func;  // 把 Base 中名为 func 的重载版本引入 Son 的作用域
...
Son son;
son.func();    // 调用 Son::func()
son.func(10);  // 调用 Base::func(int)
```
...
> 多继承容易产生二义性问题，实际开发中建议**谨慎使用**。很多语言（如 Java、C#）干脆不支持多继承。
> 
> C++ 中较常见、相对清晰的多继承场景是“一个具体类 + 多个纯接口类”。如果多个基类都携带数据成员，尤其要警惕名字冲突、初始化顺序和对象布局复杂度。
...
在一些实现中（如 MSVC），虚继承常通过**虚基类指针（vbptr）**和**虚基类表（vbtable）**来实现数据共享：
...
虚继承后，虚基类由**最底层的派生类**负责构造，中间类初始化列表里写的虚基类构造参数，在构造更底层对象时不会决定最终虚基类的构造。
...
class Alpaca : public Sheep, public Camel {
public:
    Alpaca() : Animal(3), Sheep(), Camel() {}
};

int main() {
    Alpaca a;
    cout << a.m_Age << endl;  // 3，Animal 由 Alpaca 直接构造
    return 0;
}
```
...
> 纯虚函数也可以在类外提供函数体，但类仍然是抽象类；派生类依然要覆盖纯虚函数后才能实例化。抽象基类通常应提供虚析构函数。

### ⚠️ 对象切片（Object Slicing）

多态必须通过基类指针或引用使用。如果把派生类对象按值赋给基类对象，派生类独有部分会被“切掉”，虚函数也无法表现出派生类行为：
...
int main() {
    Dog dog;
    Animal animal = dog;  // 对象切片：只保留 Animal 子对象
...
int main() {
    ...
    delete animal;  // 如果没有虚析构，这里是未定义行为，常见后果是 Dog 析构不被调用
    return 0;
}
```
...
int main() {
    ...
    // 添加不同类型的员工
    employees.push_back(make_unique<RegularEmployee>("张三", 1001, 6000, 20));
    employees.push_back(make_unique<RegularEmployee>("李四", 1002, 5500, 22));
    employees.push_back(make_unique<SalesEmployee>("王五", 2001, 4000, 50000, 0.05));
    employees.push_back(make_unique<Manager>("赵六", 3001, 15000, 5000));
