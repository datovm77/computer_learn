...
子 Agent 的核心价值不是“让更多模型同时乱改代码”，而是把复杂任务拆成可控的专业角色：有人只读分析，有人实现指定文件，有人做安全审核，有人验证测试。进阶使用 Claude Code 时，最重要的是把边界说清楚，尤其是文件所有权、写入范围、审核顺序和冲突处理。

高风险提醒：子 Agent 可以放大操作速度，也会放大误删文件、批量重构、改权限、安装依赖、提交代码、联网和处理密钥时的风险。任何涉及这些动作的任务，都应先要求只读分析和计划确认，再进入实现。
...
- 子 Agent：适合明确、聚焦的任务，由主对话分配工作，子 Agent 完成后把结果汇报回来。它适合只读探索、专项实现、代码审核、测试建议等“结果返回主线即可”的任务。
- Agent teams：适合多个 Claude Code 实例需要互相沟通、共享任务列表、协同推进的复杂工作。它是实验功能，默认关闭，会消耗更多 token，并带来更高协调成本。
...
- 子 Agent 适合把特定任务放入独立上下文，并可配置工具权限、模型和提示词。
- 子 Agent 可用于保留主对话上下文、限制能力、复用专门配置。
- Agent teams 适合需要多个 Claude Code 会话直接协作的复杂任务，但目前是实验功能，默认关闭，应谨慎使用。
